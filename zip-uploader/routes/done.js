...
const COMPLETED_BUCKET = "Completed";  // new bucket
const EXTRACTED_BUCKET = "Extracted_Files"; // source bucket
      ...
      <script>
        ...
        async function openFolder(folder) {
          ...
          document.getElementById('deleteBtn').onclick = async () => {
            if(confirm("Delete folder '"+folder+"' from Completed?")) {
              const del = await fetch('/done/'+folder+'/delete', { method:'DELETE' });
              if(del.ok){
...
router.delete("/:folder/delete", async (req, res) => {
  ...
  try {
    ...
    if (paths.length > 0) {
      const { error: delErr } = await supabase.storage.from(COMPLETED_BUCKET).remove(paths);
...
// ✅ Move folder from Extracted_Files → Completed
router.post("/:folder/done", async (req, res) => {
  ...
  try {
    ...
    // Copy each file to Completed bucket
    for (const file of data) {
      const { data: fileData, error: dlErr } = await supabase.storage.from(EXTRACTED_BUCKET).download(`${folder}/${file.name}`);
      if (dlErr) throw dlErr;

      const { error: upErr } = await supabase.storage.from(COMPLETED_BUCKET).upload(`${folder}/${file.name}`, fileData, { upsert: true });
      if (upErr) throw upErr;
    }

    // Add metadata file
    const meta = JSON.stringify({ completedAt: new Date().toISOString() });
    await supabase.storage.from(COMPLETED_BUCKET).upload(`${folder}/.completed.json`, new Blob([meta]), { upsert: true });

    // Delete from Extracted bucket
    const paths = data.map(f => `${folder}/${f.name}`);
    if (paths.length > 0) {
